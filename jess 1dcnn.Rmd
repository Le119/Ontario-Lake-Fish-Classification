...
process <- preProcess(as.data.frame(metadata_train), method=c("range"))
metadata_train <- predict(process, as.data.frame(metadata_train))

process <- preProcess(as.data.frame(metadata_test), method=c("range"))
metadata_test <- predict(process, as.data.frame(metadata_test))

process <- preProcess(as.data.frame(metadata_val), method=c("range"))
metadata_val <- predict(process, as.data.frame(metadata_val))
...
cnn_history <- model %>% fit(
  ...
  batch_size = 1000,
  epochs = 50,
  validation_data = list(list(x_val, metadata_val), dummy_y_val)
)
