...
# hidden dense layer, fully connected layers, change regularization within dense, 
# focus on activity regularizer, regularize the neuron values itself not the bias

#   layer_dense(units = 2, activation = 'sigmoid')
rnn %>%
  layer_lstm(input_shape=c(5,249), units = 249, activation = "gelu", return_sequences = TRUE) %>%
  # rnn layer, input_shape = # timepoints, units can > frequencies
  layer_lstm(units = 125, activation = "gelu") %>%
