...
library(tensorflow)
library(kernelshap)
# library(shapviz) not needed in RNN
library(str2str)
library(pROC)
...
# now just have training and testing, 5-fold cross-validation, when training separate into 5 groups and loop through each using them as validation
set.seed(15)
...
for(i in 1:20){ # subset of the different parameter that was randomly selected
...
val_loss[best_mean_val_loss[1],]      
best_epoch[best_mean_val_loss[1],]
