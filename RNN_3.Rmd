...
# our input layer
rnn %>%
    layer_dense(input_shape = dim(x.train)[2:3], 
                units = length(df.train[1,]))
rnn %>%
...
```{r}
x.train.ts = x.train
x.train.ts[[1]] = NULL # no fishNum

# Actually train our model! This step will take a while
trained_model <- rnn %>% fit(
    x = x.train.ts, # sequence we're using for prediction 
...
# installing tensorflow, using python in R
```{r}
library(reticulate)
...
# Set Python path to the correct environment
