...
```{r}
# Clearning the data
load("C:/Users/aethe/Desktop/coursebook/Course materials/STA490/processed_AnalysisData_no200.Rdata") 

processed_data_no200%>%group_by(spCode,fishNum)%>%count()

# Remove individuals with missing transducers
processed_data<-processed_data_no200%>%filter(is.na(F100)==F)

# also remove individual LWF23018, SMB004, SMB013 (only two pings)
processed_data<-processed_data%>%filter(fishNum!="LWF23018")
processed_data<-processed_data%>%filter(fishNum!="SMB004")
processed_data<-processed_data%>%filter(fishNum!="SMB013")
...
#Retain lake trout, smallmouth bass, and lake whitefish
...
fish <- fish[,c(3,52:300)]
sum(is.na(fish))

fish <- fish %>%
  mutate(spCode = case_when(spCode == 81 ~ 1,
                           spCode == 91 ~ 2,
                           spCode == 316 ~ 0))

#X-labels
frequency <- seq(45, 170, by = 0.5)
elements_to_remove <- c(90,90.5)
frequency <- frequency[!frequency %in% elements_to_remove]

#Create train and test data set
set.seed(490)
test_index_1 <-sample(which(fish$spCode == 1 ) , 1600 )
test_index_0 <-sample(which(fish$spCode == 0 ) , 1600 )
test_index <- c(test_index_0,test_index_1)
test_data <- fish[test_index,]
train_data <- fish[-test_index,]
plot_data <- train_data[,-1]

#Find scale for y-axis
max <- plot_data %>% summarise_if(is.numeric, list(~ max(., na.rm=TRUE)))
max(max)

min <- plot_data %>% summarise_if(is.numeric, list(~ min(., na.rm=TRUE)))
min(min)

write.csv(fish, "C:/Users/aethe/Desktop/coursebook/Course materials/STA490/TS_Freq_array.csv", row.names=FALSE)
```


```{r, echo=FALSE, fig.height=3, fig.width=7, warning=FALSE}
ggplot() +
    geom_line(aes(x = frequency, y = as.numeric(row_data))) +
  labs(title = "Spectrogram of target strength at each frequency",
       x = "Frequency", y = "Target Strength")
```
