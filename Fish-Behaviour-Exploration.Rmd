...
for (fish in fishes) {
  plot(processed_data[processed_data$fishNum == fish,]$pingNumber, -processed_data[processed_data$fishNum == fish,]$Target_true_depth, main = fish, ylim = c(-10, -20))
}
```

```{r}
# function to look at one specific fish
specific_fish_ping_lag <- function(fishCode, indicator_n = 1000){
