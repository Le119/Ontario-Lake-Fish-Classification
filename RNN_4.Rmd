...
*-trying to train model without removing dead fish*
...
processed_data<-processed_data%>%filter(spCode!="102") # filter out spCode 102
...
```
SMB=313. 
LT=784, want 313, need remove 471. 
LWF=324, want 313, need remove 11. 
```{r}
...
set.seed(5)
rem<-sample(c(rep(0,471),rep(1,313)),784)
...
data_0.5sec_validate[,5:249]<-exp(data_0.5sec_validate[,5:249]/10) # dimension need match x_train_data dimension
...
# remove 108 LT & 43 LWF
set.seed(5)
rem<-sample(c(rep(0,108),rep(1,62)),170)
rem2<-sample(c(rep(0,43),rep(1,62)),105)
rem3<-rep(1,62) # SMB kept the same so no sampling for removal
...
data_0.5sec_test[,5:249]<-exp(data_0.5sec_test[,5:249]/10) # 249=#frequencies
...
set.seed(5)
rem<-sample(c(rep(0,125),rep(1,12)),137)
rem2<-rep(1,12)
rem3<-sample(c(rep(0,67),rep(1,12)),79)
remove<-which(c(rem,rem2,rem3)==0)
